...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .xlegend{display:flex;flex-wrap:wrap;gap:8px 12px;margin-bottom:14px}
  .xchip{display:flex;align-items:center;gap:6px;color:#dbe8ff;font-size:11px;font-weight:800;
    letter-spacing:1px;text-transform:uppercase;background:#ffffff22;border-radius:8px;padding:7px 10px}
  .xchip i{width:9px;height:9px;border-radius:50%;background:var(--cc);flex:0 0 auto}
  ...
  .xcells{display:flex;gap:10px;flex:1;min-width:0}
  .xcell{width:67px;flex:0 0 auto;aspect-ratio:1;border-radius:12px;display:grid;place-items:center;position:relative;overflow:hidden}
  .xcell.xmissing{background:#ffffff1a;box-shadow:0 0 0 2px var(--cc)4d}
  ...
  try{
    ...
    const rowsHtml = ROWS.map(r=>{
      ...
      for(const v of V){
        ...
        else { cellsHtml+=`<div class="xcell xmissing" style="--cc:${VC[v]}"><img src="${file}"></div>`; }
      }
...
function discordText(){
  const MARK={master:"👑",own:"✅",lost:"👻",need:"❌"};
  const L=[];
  L.push("✅Have 👑Mastered 👻Lost ❌Need");
  L.push("");
  let coll=0, tot=0;
  for(const r of ROWS){
    const cells=[];
    for(const v of V){
      ...
      tot++;
      const s=st(r.key,v);
      if(s==="master"||s==="own") coll++;
      cells.push(v+(MARK[s]||MARK.need));
    }
